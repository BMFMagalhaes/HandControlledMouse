...
static void setImage(const LEAP_IMAGE_EVENT* image_event);

//External state
bool IsConnected = false;
String currentVolunteerID;
...
void updateVolunteerId(std::string currID) {
    currentVolunteerID = currID;
}
...
void updateTrialId(int trialID) {
    currentTrialID = trialID;
}

void updateSuggested(bool value) {
    IsSuggested = value;
}
...
static void OnImage(const LEAP_IMAGE_EVENT* imageEvent) {
    ...
    const void* imageData = static_cast<const char*>(imageEvent->image[0].data) + imageEvent->image[0].offset;
    int imageSize = imageEvent->image[0].properties.width * imageEvent->image[0].properties.height;
    const cv::Mat mat = cv::Mat((int)imageEvent->image[0].properties.height, (int)imageEvent->image[0].properties.width, CV_8UC1, (void*)imageData);
    std::ostringstream filename;
    filename << currentQuestionID << "-" << currentTrialID << "-" << imageEvent->info.timestamp << ".png";
    char full_path[50];
    const char* folderPath = "C:\\teste\\";
    strcpy_s(full_path, folderPath);
    strcat_s(full_path,std::to_string(imageEvent->info.frame_id).c_str());
    //int status = _mkdir(full_path);
    strcat_s(full_path, ".png");
    //strcat_s(full_path, filename.str().c_str());
    //printf("%s", full_path);
    imwrite(full_path, mat);
}
...
void setImage(const LEAP_IMAGE_EVENT* image_event) {
    LockMutex(&dataLock);
    if (!lastImage) lastImage = (LEAP_IMAGE_EVENT*)malloc(sizeof(*image_event));
    *lastImage = *image_event;
